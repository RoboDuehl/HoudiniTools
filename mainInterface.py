...
class FolderInterface:
    def __init__(self):
        ...
        #unizip button
        self.unzip_btn_image = PhotoImage(file=r"buttons\unzip.png")
        self.unzip_btn = Button(bg=THEME_COLOR, image=self.unzip_btn_image, bd=0, highlightbackground=THEME_COLOR, highlightthickness=0, command=self.unzipAllBtn)
        self.unzip_btn.grid(row=5, column=1)    
        
        #csv exporter
        self.csv_btn_image = PhotoImage(file=r"buttons\csv.png")
        self.csv_btn = Button(bg=THEME_COLOR, image=self.csv_btn_image, bd=0, highlightbackground=THEME_COLOR, highlightthickness=0, command=self.exportCsv)
        self.csv_btn.grid(row=6, column=1)    
        
        
        #delete button
        self.delete_btn_image = PhotoImage(file=r"buttons\delete.png")
        self.delete_btn = Button(bg=THEME_COLOR, image=self.delete_btn_image, bd=0, highlightbackground=THEME_COLOR, highlightthickness=0, command=self.deleteZipFiles)
    ...
    def openDirectory(self):
        ...
        self.central_canvas.itemconfig(self.central_text, text=f"{len(self.zip_files)} zip files found in: {self}")
